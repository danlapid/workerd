...
kj::Promise<void> DawnRemoteSerializer::handleIncomingCommands() {
  for (;;) {
    ...
    if (n == 0) {
      KJ_DBG("EOF received while reading from stream");
      co_return;
    }

    KJ_DBG("read bytes from stream", n, _rbuf.len());
    if (_dawnCmdRLen > 0) {
...
kj::Promise<void> DawnRemoteSerializer::actualFlush() {
  ...
  if (_dawnout.flushlen != 0) {
    ...
    KJ_DBG("_dawnout flush", _dawnout.flushoffs, len);
...
bool DawnRemoteSerializer::readMsg() {
  ...
  while (_rbuf.len() > 0) {
    ...
    case MSGT_DAWNCMD:
      KJ_DBG("MSGT_DAWNCMD", _rbuf.len(), _rbuf.at(0));
...
bool DawnRemoteSerializer::maybeReadIncomingDawnCmd() {
  ...
  if (buf == nullptr) {
    ...
    KJ_DBG("data is not contiguous, will copy to temporary buffer _dawntmp");
...
bool DawnRemoteSerializer::Flush() {
  KJ_DBG("Flush() was called", _dawnout.writelen, _dawnout.flushlen);
